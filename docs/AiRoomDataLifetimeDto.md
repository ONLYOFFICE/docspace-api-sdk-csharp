# DocSpace.API.SDK.Model.AiRoomDataLifetimeDto
...
**Period** | **AiRoomDataLifetimePeriod** | The unit the age is counted in. Months and years are counted as calendar ones, so the same number of them  covers a different number of days depending on when the clean-up runs. | [optional] 
