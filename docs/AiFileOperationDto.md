# DocSpace.API.SDK.Model.AiFileOperationDto
...
**Operation** | **AiFileOperationType** | What the operation does with the entries, which also decides what else is reported: only a download fills  `url`, and a deletion leaves `files` and `folders` empty. | 
...
**Status** | **AiDistributedTaskStatus** | The state of the background task behind the operation, which tells a task that was cancelled or that crashed  from one that ran to its end. | [optional] 
